...
官网和历史资料可能仍包含旧版依赖、Flyway、`VITE_API_URL` 或过时接口风格。涉及具体事实、接口或配置时，先检索当前代码；不要为与任务无关的内容读取资料。
...
2. 根据任务读取最小必要资料：页面任务看相关页面和组件，API 或跨端字段任务再看类型、API、配置及后端契约。
3. 仅在任务涉及对应场景时，记录受影响的页面、接口、权限、字典、菜单、上传、环境变量或代理。
...
- 优先复用当前目录、组件、Hook 和调用模式；仅在对应场景需要时查阅其 API，不创建重复能力。
- 变量名、函数名使用英文；新增代码注释、错误信息和日志说明使用中文。
- 避免新增 `any`；优先定义准确的 TypeScript 类型。若第三方边界必须使用 `any`，应限制在最小范围并说明原因。
- 未经用户要求，不创建提交或推送；开发服务只为必要的短时验证启动，并在验证后停止。

### 2.3 完成后

1. 实现任务要求，运行受影响范围内的最小验证，并修复由本次改动导致的失败。
2. 代码或文档发生修改时，执行 `git diff --check`，再复查 `git diff` 和 `git status --short`。
3. 最终说明列出修改文件与行为、已执行验证、未执行验证及原因；无后端配合或契约变化时明确写“无”。
...
开发 CRUD 时按实际场景查阅 `src/components/` 与 `src/hooks/` 的 API：表格或搜索使用 `ProTable`、`SearchForm`、`useTable`；选择或批量操作使用 `useSelection`、`useHandleData`；字典使用 `useDict`、`useDictOptions`；上传、导入、导出和弹窗宽度分别使用对应组件、`ImportExcel`、`useDownload`、`useDialogWidth`。
...
HTTP 方法和 URL 必须以目标模块当前 Controller 契约为准；这是本项目对新增或调整接口的优先规则，覆盖通用的请求方法偏好。仓库同时存在 GET、POST、PUT、DELETE 的存量接口，不得从旧官网推断或脱离后端契约统一改写。仅在接口任务中核对方法、路径、参数位置和响应类型。
...
调整 API 前缀或部署路径时，按实际改动核对相关 `.env*`、`src/api/client.ts`、`vite.config.mts`、`nginx/default.conf` 和后端契约。
...
修改以下任一跨端字段时，必须核对前后端及初始化数据；普通页面或样式任务不需要读取这些资料：
...
生产镜像使用 Nginx，静态目录、监听端口和代理规则以 `Dockerfile`、`nginx/default.conf` 为准。仅在部署、API 代理或 WebSocket 配置任务中核对实际相关项：
...
| 多文件逻辑或需要规则检查的改动   | `pnpm lint`，并复查自动修复结果                              |
| 依赖、构建、路由、模块注册       | `pnpm build`                                               |
| 环境变量、API 前缀或部署配置     | 核对实际相关配置、代理和后端契约；必要时再运行 `pnpm build` |
| 明显交互或路由影响的 UI 改动     | `pnpm build-only`；可运行时短时启动 `pnpm dev` 验证关键路径 |
| 仅文档                         | 检查 Markdown 结构、链接和 `git diff --check`              |

未执行的检查必须在交付说明中列明原因，不得把未执行、被跳过、仍在运行或仅静态检查的项目描述为验证通过。任务在实现要求完成、受影响验证通过或已说明阻塞原因、并完成 diff 复查后结束；无需等待长期服务或无关的全量检查。
...
- 仅当界面改动需要视觉确认或用户要求时，在 PR 或交付说明中附关键页面截图；配置、依赖和契约改动说明兼容性、后端配合点与回滚方式。
